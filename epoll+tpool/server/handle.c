...
int handle_show(int cfd,struct send_pack recv)
{
	...
	switch(recv.result)
	{
		...
		case 4:
		{
			...
			search_by_id(db,"friendreq","id2",recv.send_id1,INPORT);
			p = cb_head->next;
			while(p != NULL)
			{
				if(strcmp(p->dec,"unchecked") == 0)
				{
					memset(&msg_pack,0,sizeof(msg_pack));
...
int handle_login(int cfd,struct send_pack recv_pack)
{
    ...
    if(ret == 1)
    {
    	if(memcmp(recv_pack.send_msg0,(cb_head->next)->pswd,MAX_SIZE) == 0)
    	{
			Link new = NULL;
			init_node(&new);
			new->fd = cfd;
     	...
     	{
     		sprintf(sdpk.send_msg0,"# Password incorrect!");
        	sdpk.result = FAIL;
        	send_pk(&sdpk,cfd);
     	}
	}
    ...
    {
    	sprintf(sdpk.send_msg0,"# User name does not exist!");
        sdpk.result = FAIL;
        send_pk(&sdpk,cfd);
    }
}
...
int handle_accept_friend(int cfd,struct send_pack recv_pk)
{
	int count = 0;
	Link p;
	Link q;
	...
	char buf[MAX_SIZE] = {0};
	struct send_pack sdpk;
	struct send_pack tempdata;
	
	p = online_user->next;

	search_by_id(db,"friendreq","id2",recv_pk.send_id1,INPORT);
	q = cb_head->next;

	while(q != NULL)
	{
		if((q->id1 == recv_pk.send_id2) && (strcmp(q->dec,"unchecked") == 0))
		{
			memset(&tempdata,0,sizeof(tempdata));
			search_by_id(db,"userdata","id1",recv_pk.send_id2,EXCHANGE);
		
			tempdata.send_id1 = recv_pk.send_id2;
			strcpy(tempdata.send_msg0,"friend");
			strcpy(tempdata.send_msg1,temp_name);
			insert_data(db,recv_pk.sourse_name,tempdata,6);
			...
			memset(&tempdata,0,sizeof(tempdata));
			tempdata.send_id1 = recv_pk.send_id1;
			strcpy(tempdata.send_msg0,"friend");
			strcpy(tempdata.send_msg1,recv_pk.sourse_name);
			insert_data(db,temp_name,tempdata,6);

			while(p != NULL)
			{
				if(p->id1 == recv_pk.send_id2)
				{
					memset(&sdpk,0,sizeof(sdpk));
					sprintf(sdpk.send_msg0,"# %s has been your friend!",recv_pk.sourse_name);
					sdpk.send_cmd = INFO;
					send_pk(&sdpk,p->fd);
					break;
				}
			...
			sprintf(buf,"%d",q->id3);
			update_data(db,"friendreq","dec","checked","id3",buf,3);
			count++;
			break;
		}
		q = q->next;
	...
	{
		memset(&sdpk,0,sizeof(sdpk));
		sprintf(sdpk.send_msg0,"# Accpet friend fail!");
		sdpk.send_cmd = INFO;
		send_pk(&sdpk,cfd);
	}
}

int handle_add_friend(int cfd,struct send_pack recv_pk)
{
	extern sqlite3 *db;
	extern Link online_user;
	extern Link cb_head;
	...
	memset(&sdpk,0,sizeof(sdpk));
	search_by_id(db,"userdata","id1",recv_pk.send_id2,EXCHANGE);
	search_data(db,recv_pk.sourse_name,"dec","friend",INPORT);
	q = cb_head->next;
	while(q != NULL)
	{
		if(q->id1 == recv_pk.send_id2)
		{
			flag++;
			break;
		}
	...
	if(flag == 0)
	{
		count = search_data(db,"friendreq",NULL,NULL,COUNT);
		memset(&temp_data,0,sizeof(temp_data));
		strcpy(temp_data.send_msg0,"unchecked");
		temp_data.send_id1 = recv_pk.send_id1;
		temp_data.send_id2 = recv_pk.send_id2;
		temp_data.send_id3 = count + 1;
		...
		while(p != NULL)
		{
			...
			{
				sdpk.send_cmd = INFO;
				sprintf(sdpk.send_msg0,"%s wants to be your friend !",recv_pk.sourse_name);
				send_pk(&sdpk,p->fd);
				break;
			}
			p = p->next;
...
int handle_group(int cfd,struct send_pack recv)
{
	...
	switch(recv.result)
	{
		case 1:
		{
			memcpy(tempdata.sourse_name,recv.send_msg0,sizeof(recv.send_msg0));
			count = search_data(db,"grouplist",NULL,NULL,COUNT);
			tempdata.send_id1 = count + 1;
			insert_data(db,"grouplist",tempdata,2);

			sprintf(sdpk.send_msg0,"# Group %s has successfully created ! ,id is %d",tempdata.sourse_name,count + 1);
			sdpk.send_cmd = INFO;
			send_pk(&sdpk,cfd);

			memset(&tempdata,0,sizeof(tempdata));
			create_table(db,recv.send_msg0,6);
			strcpy(tempdata.send_msg0,"member");
			strcpy(tempdata.send_msg1,recv.sourse_name);
			tempdata.send_id1 = recv.send_id1;
			insert_data(db,recv.send_msg0,tempdata,6);

			memset(&tempdata,0,sizeof(tempdata));
			strcpy(tempdata.send_msg0,"group");
			tempdata.send_id1 = count + 1;
			memcpy(tempdata.send_msg1,recv.send_msg0,strlen(recv.send_msg0));
			insert_data(db,recv.sourse_name,tempdata,6);
			break;
		}
		...
		case 2:
		{
			...
			p = cb_head->next;
			sprintf(sdpk.send_msg0,"\n|%-10s|%-10s|\n","groupid","groupname");
			sdpk.send_cmd = INFO;
			send_pk(&sdpk,cfd);
			while(p != NULL)
			{
				memset(&sdpk,0,sizeof(sdpk));
				sprintf(sdpk.send_msg0,"|%-10d|%-10s|",p->id1,p->name);
				sdpk.send_cmd = INFO;
				send_pk(&sdpk,cfd);
				p = p->next;
			}
		...
		case 3:
		{
			count = search_data(db,"grouplist","name",recv.send_msg0,COUNT);
			if(count == 1)
			{
				strcpy(tempdata.send_msg0,"member");
				tempdata.send_id1 = recv.send_id1;
				memcpy(tempdata.send_msg1,recv.sourse_name,strlen(recv.sourse_name));
				insert_data(db,recv.send_msg0,tempdata,6);

				memset(&tempdata,0,sizeof(tempdata));
				search_data(db,"grouplist","name",recv.send_msg0,INPORT);
				strcpy(tempdata.send_msg0,"group");
				tempdata.send_id1 = (cb_head->next)->id1;
				memcpy(tempdata.send_msg1,recv.send_msg0,strlen(recv.send_msg0));
				insert_data(db,recv.sourse_name,tempdata,6);

				sprintf(sdpk.send_msg0,"# User %s has join into group %s !",recv.sourse_name,recv.send_msg0);
				memset(&sdpk,0,sizeof(sdpk));
				sdpk.send_cmd = INFO;
				send_pk(&sdpk,cfd);
			}
			...
			{
				memset(&sdpk,0,sizeof(sdpk));
				strcpy(sdpk.send_msg0,"# Invaild group name !");
				sdpk.send_cmd = INFO;
				send_pk(&sdpk,cfd);
			}
			break;
		...
		case 8:
		{
			count = search_data(db,recv.sourse_name,"dec","group",INPORT);
			p = cb_head->next;
			sprintf(sdpk.send_msg0,"\n|%-10s|%-10s|","id","name");
			sdpk.send_cmd = INFO;
			send_pk(&sdpk,cfd);
			...
			{
				memset(&sdpk,0,sizeof(sdpk));
				sprintf(sdpk.send_msg0,"|%-10d|%-10s|",p->id1,p->message);
				sdpk.send_cmd = INFO;
				send_pk(&sdpk,cfd);
				p = p->next;
			}
		...
		case 9:
		{
			...
			count = search_data(db,recv.send_msg0,"dec","member",INPORT);
			p = cb_head->next;
			sprintf(sdpk.send_msg0,"\n|%-10s|%-10s|","id","name");
			sdpk.send_cmd = INFO;
			send_pk(&sdpk,cfd);
			...
			while(p != NULL)
			{
				memset(&sdpk,0,sizeof(sdpk));
				search_by_id(db,"userdata","id1",p->id1,EXCHANGE);
				sprintf(sdpk.send_msg0,"|%-10d|%-10s|",p->id1,temp_name);
				sdpk.send_cmd = INFO;
				send_pk(&sdpk,cfd);
				p = p->next;
			}
...
int handle_delete_friend(int cfd,struct send_pack recv)
{
	extern sqlite3 *db;
	extern Link cb_head;
	extern Link online_user;
	...
	memset(&sdpk,0,sizeof(sdpk));
	count = search_data(db,"userdata","id1",recv.send_id2,EXCHANGE);
	if(count == 1)
	{
		delete_data(db,temp_name,"message",recv.sourse_name);
		delete_data(db,recv.sourse_name,"message",temp_name);
		p = online_user->next;
		while(p != NULL)
		{
			if(p->id1 == recv.send_id2)
			{
				sdpk.send_cmd = INFO;
				sprintf(sdpk.send_msg0,"# You have lost a friend named %s!",recv.sourse_name);
				send_pk(&sdpk,p->fd);
				break;
			}
			p = p->next;
		...
		memset(&sdpk,0,sizeof(sdpk));
		sdpk.send_cmd = INFO;
		sprintf(sdpk.send_msg0,"# You have lost a friend named %s!",temp_name);
		send_pk(&sdpk,cfd);
	}
}
