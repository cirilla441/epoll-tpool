...
int create_table(sqlite3 *db,char *table_name,int cmd)
{
	...
	switch(cmd)
	{
		...
		case 5:
		{
			sprintf(sql,"create table if not exists %s(dec text,id1 integer,id2 integer,id3 integer);",table_name);//dec决定是好友还是群
			break;
		}
...
int insert_data(sqlite3 *db,char *table_name,struct send_pack recv,int cmd)
{
	...
	switch(cmd)
	{
		...
		case 5:
		{
			sprintf(sql,"insert into %s(dec,id1,id2,id3) values('%s',%d,%d,%d);",table_name,recv.send_msg0,recv.send_id1,recv.send_id2,recv.send_id3);//friend id group id
			break;
		}
		...
		{
			//msg0 id1 msg1
			//frined/group friendid/groupid name
			//message/member id content/name
